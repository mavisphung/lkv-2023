...
import { Accordion, Avatar,Card, Container, createStyles, Grid, Group, rem, Text, useMantineTheme } from "@mantine/core";
...
export const products: Product[] = [
  {
    ...
    imgURL: ["https://www.offidocs.com/images/oceancovesea.jpg",
...
const ProductDetailPage = () => {
  ...
  let product = products.find(product => product.id.toString() === params.productId)
    ...
    <>
      <Container size="xl">
        <div className={classes.breadcrumb}>
          {mobile ? <Link className={classes.link} to={AppRoutes.products}><Text c="dimmed">{product?.categoryName}</Text></Link> :
          <>
          <Link className={classes.link} to={AppRoutes.products}><Text c="dimmed">Sản phẩm /</Text></Link>
          <Link className={classes.link} to={AppRoutes.products}><Text c="dimmed">{product?.categoryName} /</Text></Link>
          <Text className={classes.link}>{product?.name}</Text></>}</div>
        <Grid gutter={mobile ? 0 : rem(28)} >
          <Grid.Col xs={12} sm={6} ref={ref}>
            <Avatar className={classes.product_img} src={product?.imgURL[imgIndex]} size={width} radius="md" />
            <Carousel slideGap="lg" slideSize={width / 4}
              align="start" slidesToScroll={mobile ? 1 : 2}
              controlsOffset="xs" controlSize={28}
              styles={{
                control: {
                  '&[data-inactive]': {
                    opacity: 0,
            ...
            >
              {product?.imgURL.map((el, idx) => (
                <Carousel.Slide>
                  <Avatar key={idx} src={el}
                    size={width / 5} radius="sm"
                    className={cx(classes.img_row, { [classes.active]: (idx === imgIndex) })}
                    onClick={() => setImgIndex(idx)} />
                </Carousel.Slide>))}
            </Carousel>
          </Grid.Col>
          <Grid.Col xs={12} sm={6} style={{ margin: `${rem(36)} 0 ${rem(36)} 0` }} >

            <Text className={classes.product_name}>{product?.name}</Text>
            {/* product status */}
            {/* not responsive yet */}
            <Grid.Col xs={3} sm={4.5} xl={3} span={mobile ? 4 : 0} >
              <Card className={classes.product_status} padding="sm" m={0} mt={mobile ? "1em" : 0} >
            ...
            <Card children={
              <Group style={{ justifyContent: "space-evenly" }}>
                <Text className={`${classes.price} ${classes.title}`}>742.000 đ - 830.000 đ</Text>
                <Grid.Col span={3}>
            ...
            <Accordion>
              <Accordion.Item value="1">
                ...
                <Accordion.Panel>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                </Accordion.Panel>
              </Accordion.Item>
          ...
          {/* related products */}
          <Grid.Col><Text className={classes.title} size={rem(20)} mt={rem(9)}>Sản phẩm liên quan</Text>
          </Grid.Col>
          <Grid.Col><Group className={classes.related_products}>
            {products.map((el) => <ProductCard width={width / 2 - 14} {...el} />)}
          </Group></Grid.Col>
